...
import { C as onSnapshot, M as doc, O as updateDoc, P as serverTimestamp, T as query, j as collection, k as where, x as getDocs, y as getCountFromServer } from "../_libs/@firebase/firestore+[...].mjs";
...
import { D as ShoppingBag, Lt as Check, a as Watch } from "../_libs/lucide-react.mjs";
import { n as generateQrDataUrl, t as buildPortalUrl } from "./qr-CQKCS1pc.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/dashboard-D57V_OzL.js
...
function Dashboard() {
	...
	(0, import_react.useEffect)(() => {
		...
		const resetStart = () => {
			if (shop.dailyResetMode === "manual" && shop.lastManualResetAt) return shop.lastManualResetAt;
		...
		(async () => {
			try {
				...
				const histOrders = allOrdersList.filter((o) => {
					return (o.createdAt?.toMillis?.() ?? 0) < startTime.getTime() && o.status !== "Waiting";
				});
				...
				const todayQ = query(collection(db, COL.orders), where("shopId", "==", shop.id), where("createdAt", ">=", startTime));
				...
				const updateLiveStats = () => {
					...
					const liveTodayOrders = orderRows.filter((r) => {
						return (r.createdAt?.toMillis?.() ?? 0) >= cutoff && r.status !== "Waiting";
					});
					...
					setStats((prev) => ({
						...
						completed: orderRows.filter((r) => r.status === "completed" && (r.createdAt?.toMillis?.() ?? 0) >= cutoff).length
					}));
				};
	...
	const resetEndOfDay = async () => {
		try {
			await updateDoc(doc(getDb(), COL.shops, shop.id), { lastManualResetAt: serverTimestamp() });
			setStats((prev) => ({
				...prev,
				today: 0,
				revenue: 0,
								...
								children: [
									...
									/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
										...
										children: [
											...
											isManualReset && /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Button, {
