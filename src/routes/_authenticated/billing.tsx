...
function MobileBilling({
  ...
  return <section className="md:hidden pb-28">
    <header className="sticky top-0 z-20 -mx-3 mb-4 border-b border-slate-100 bg-white/95 px-3 pb-3 pt-2 backdrop-blur">
      ...
      <div className="no-scrollbar mt-3 flex gap-2 overflow-x-auto pb-1">
        <button onClick={() => setSelectedCategoryId(null)} className={`min-h-12 shrink-0 rounded-full px-4 text-sm font-bold ${selectedCategoryId === null ? "bg-orange-500 text-white" : "bg-slate-100 text-slate-700"}`}>All</button>
        {categories.map((category: Category) => <button key={category.id} onClick={() => setSelectedCategoryId(category.id)} className={`min-h-12 shrink-0 rounded-full px-4 text-sm font-bold ${selectedCategoryId === category.id ? "bg-orange-500 text-white" : "bg-slate-100 text-slate-700"}`}>{category.name}</button>)}
      </div>
    </header>
